...
<c:import url="../layout/app.jsp">
  <c:param name="content">
    ...
    <h2>「<c:out value="${comicTitle}" /> 」<span>のレビュー</span></h2>
       ...
       <tbody>
          ...
          <c:forEach var="review" items="${reviews}" varStatus="status">
             <tr class="row${status.count%2} tableBody">
                ...
                <td Class="review_Title"><a href="<c:url value='/reviews/onlyShow?id=${review.id}' />">${review.title}</a></td>
     ...
     <div id="pagination">
            ...
            <c:forEach var="i" begin="1" end="${((reviews_count - 1) / 15) + 1}" step="1">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="<c:url value='/reviews/onlyIndex?page=${i}&id=${sessionScope.comic_id}' />"><c:out value="${i}" /></a>&nbsp;
                    </c:otherwise>
                </c:choose>
            </c:forEach>
       </div>

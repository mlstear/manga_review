...
<c:import url="../layout/app.jsp">
  <c:param name="content">
    ...
    <h2><c:out value="${sessionScope.login_poster.name}" />さん　マイページ</h2>
     ...
     <div id="pagination">
            ...
            <c:forEach var="i" begin="1" end="${((reviews_count - 1) / 15) + 1}" step="1">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="<c:url value='/reviews/myIndex?page=${i}' />"><c:out value="${i}" /></a>&nbsp;
                    </c:otherwise>
                </c:choose>
            </c:forEach>
       </div>
       <p><a href="<c:url value='/posters/myShow' />">投稿者詳細を見る</a></p>
